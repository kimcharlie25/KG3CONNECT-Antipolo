        ...
        <div className="relative h-48 bg-gradient-to-br from-gray-50 to-gray-100">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {item.isOnDiscount && item.discountPrice && (
              <div className="bg-gradient-to-r from-kg3-orange to-red-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg animate-pulse">
                PROMO
              </div>
            )}
            {item.popular && (
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
                ⭐ POPULAR
              </div>
            )}
          </div>
          ...
          {!item.available && (
            <div className="absolute top-3 right-3 bg-gray-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
              UNAVAILABLE
            </div>
          )}
          ...
          {item.isOnDiscount && item.discountPrice && (
            <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm text-kg3-orange text-xs font-bold px-2 py-1 rounded-full shadow-lg">
              ...
              {item.isOnDiscount && item.discountPrice ? (
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <span className="text-2xl font-bold text-kg3-orange">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex items-center justify-between rounded-t-2xl">
              <div>
                <h3 className="text-xl font-bold text-gray-900">Customize {item.name}</h3>
            ...
            <div className="p-6">
              ...
              {item.variations && item.variations.length > 0 && (
                <div className="mb-6">
                  <h4 className="font-semibold text-gray-900 mb-4">Choose Option</h4>
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <input
                            ...
                            className="text-kg3-orange focus:ring-kg3-orange"
                          />
                          ...
                          >
                            ...
                            <div className="flex items-center space-x-2">
                              {selectedAddOns.find(a => a.id === addOn.id) ? (
                                <div className="flex items-center space-x-2 bg-orange-100 rounded-xl p-1 border border-orange-200">
                                  <button
                                    type="button"
                                    onClick={() => {
                                    ...
                                    className="p-1.5 hover:bg-orange-200 rounded-lg transition-colors duration-200"
                                  >
                                    <Minus className="h-3 w-3 text-kg3-orange" />
                                  </button>
                                  ...
                                  <button
                                    ...
                                    className="p-1.5 hover:bg-orange-200 rounded-lg transition-colors duration-200"
                                  >
                                    <Plus className="h-3 w-3 text-kg3-orange" />
                                  </button>
                                </div>
                              ) : (
                                <button
                                  ...
                                  className="flex items-center space-x-1 px-4 py-2 bg-kg3-orange text-white rounded-xl hover:opacity-90 transition-all duration-200 text-sm font-medium shadow-lg"
                                >
              ...
              <div className="border-t border-gray-200 pt-4 mb-6">
                <div className="flex items-center justify-between text-2xl font-bold text-gray-900">
                  <span>Total:</span>
                  <span className="text-kg3-orange">₱{calculatePrice().toFixed(2)}</span>
                </div>
              </div>

              <button
                onClick={handleCustomizedAddToCart}
                className="w-full bg-kg3-orange text-white py-4 rounded-xl hover:opacity-90 transition-all duration-200 font-semibold flex items-center justify-center space-x-2 shadow-lg hover:shadow-xl transform hover:scale-105"
              >
                <span>Confirm Selection - ₱{calculatePrice().toFixed(2)}</span>
              </button>
            </div>
          </div>
        </div>
